...
#     planetengine.log("Iterating model2 by 10 steps...")

#     model2.go(10)

#     planetengine.log("Checkpointing model2...")
...
#             os.remove(model3.tarpath)
    ...
    planetengine.log("Diagnostic complete!")

    if delete:
        if rank == 0:
            os.remove('diaglog.txt')

def diagnostic_02(delete = True):
    ...
    model1 = planetengine.frame.Frame(
        planetengine.diagnostic.MS98_systemscript.build(res = 16, f = 1., tau = 1e5),
        planetengine.diagnostic.MS98_observerscript.build(),
        initial
        )
    ...
    model2 = planetengine.frame.Frame(
        planetengine.diagnostic.MS98_systemscript.build(res = 32, f = 0.5, tau = 4e5),
        planetengine.diagnostic.MS98_observerscript.build(),
        initial
        )
...
#     model2.reset()
    ...
    model3 = planetengine.frame.Frame(
        planetengine.diagnostic.MS98_systemscript.build(res = 64, f = 1., tau = 1e6),
        planetengine.diagnostic.MS98_observerscript.build(),
        initial
        )
